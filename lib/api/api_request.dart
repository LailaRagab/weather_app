...
class ApiRequest{
 ...
 Future <WeatherModel> getCurrentWeather({required String cityName}) async{
    try {
      Response response= await  dio.get('$base/forecast.json?key=$apiKey&q=$cityName&days=1');

      WeatherModel weatherModel = WeatherModel.fromJson(response.data);
      return weatherModel;
      // handling errors from dio request
    } on DioException catch (e) {
      final String badResponse = e.response?.data['error']['message'] ?? "oops there 's an error !" ;
